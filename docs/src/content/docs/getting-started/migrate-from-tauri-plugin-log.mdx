...
Commands continue to work without changes. For high-value commands, use `auditaur_command` so frontend invoke spans connect to backend command spans:

```rust
#[tauri_plugin_auditaur::auditaur_command]
fn render_video(id: String) -> Result<(), String> {
