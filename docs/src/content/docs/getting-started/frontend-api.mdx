...
`driveBridge` is an explicit opt-in debug/test feature. When enabled, the frontend polls the Auditaur plugin for local drive requests, runs selector operations inside the WebView, and writes responses back to the Auditaur session directory. This gives `auditaur drive` Tauri-native selector semantics on every platform without a browser debugging endpoint. Keep it disabled for normal application builds; enable it only for local dogfood, CI, or agent-driven smoke tests:
