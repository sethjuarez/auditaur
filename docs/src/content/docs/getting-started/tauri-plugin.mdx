...
The frontend wrapper sends W3C `traceparent` in the reserved `auditaurTraceContext` argument. The backend helper validates it and exposes it to Auditaur's tracing layer, so the backend command span is parented to the frontend invoke span without command-name or timestamp heuristics.
