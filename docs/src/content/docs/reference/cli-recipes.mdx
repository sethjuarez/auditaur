...
## Focus evidence around one generic anchor
...
Anchors keep the product generic: use `trace:<id>`, `ipc:<command-or-latest>`, `error:latest`, `event:<name>`, `checkpoint:<name>`, or `time:<rfc3339-or-unix-nanos>` instead of adding app-specific lenses.
...
## Live-tail only failure signals
