...
```powershell
auditaur debug --app my-tauri-app --active --require-frontend --json run --timeout-seconds 180 -- npm run tauri dev
```

Use wrapper mode when an agent or script owns startup but should leave the app running after readiness for inspection.
