...
8. `.github/workflows/release-binaries.yml` uploads Windows x64, Windows ARM64, macOS ARM64, and Linux x64 `auditaur` binaries for `auditaur-cli-v*` GitHub releases.
...
5. Run `.\scripts\preflight-release.ps1` before publishing release PRs or binaries. Use `-IncludeDogfoodSmoke` when a live Windows dogfood pass is appropriate.
